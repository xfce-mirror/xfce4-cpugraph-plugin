...
constexpr auto g_per_core_bars = "/per-core-bars";
...
Settings::read (XfcePanelPlugin *plugin, const shared_ptr<CPUGraph> &base)
{
    ...
    if (const auto chn = base->channel)
    {
        ...
        if (const auto rc_file = xfce_panel_plugin_lookup_rc_file (plugin))
        {
            ...
            if (migrate)
            {
                if (const auto rc = xfce4::Rc::simple_open (rc_file, true))
                {
                    ...
                    per_core_bars = rc->read_int_entry ("PerCoreBars", per_core_bars);
        ...
        if (!migrate)
        {
            ...
            per_core_bars = xfconf_channel_get_int (chn, g_per_core_bars, per_core_bars);
    ...
    base->set_per_core_bars (per_core_bars);
...
Settings::write (XfcePanelPlugin *plugin, const shared_ptr<const CPUGraph> &base)
{
    ...
    xfconf_channel_set_int (chn, g_per_core_bars, base->per_core_bars);
