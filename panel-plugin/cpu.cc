...
{
    auto tooltip = xfce4::sprintf (_("CPU usage: %.1f%%"), base->cpu_data[base->index_to_cpu[0]].load * 100.0f);
...
draw_bars_cb (cairo_t *cr, const shared_ptr<CPUGraph> &base)
{
    ...
    {
        gfloat usage = base->cpu_data[base->index_to_cpu[0]].load;
