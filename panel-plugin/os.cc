...
read_cpu_data (std::vector<CpuData> &data)
{
    ...
    static std::vector<gulong> system_arr, user_arr, nice_arr, iowait_arr, total_arr;
    if (system_arr.size() < nb_cpu) system_arr.resize(nb_cpu);
    if (user_arr.size() < nb_cpu) user_arr.resize(nb_cpu);
    if (nice_arr.size() < nb_cpu) nice_arr.resize(nb_cpu);
    if (iowait_arr.size() < nb_cpu) iowait_arr.resize(nb_cpu);
    if (total_arr.size() < nb_cpu) total_arr.resize(nb_cpu);
