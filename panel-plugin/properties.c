...
create_options (XfcePanelPlugin *plugin, CPUGraph *base)
{
    ...
    gtk_box_pack_start (vbox2, gtk_separator_new (GTK_ORIENTATION_HORIZONTAL), FALSE, FALSE, BORDER/2);
    dlg_data->hbox_highlight_smt = create_check_box (vbox2, sg, _("Highlight suboptimal SMT scheduling"),
                                                     base->highlight_smt, change_smt, dlg_data);
    setup_color_option (vbox2, sg, dlg_data, SMT_ISSUES_COLOR, _("SMT issues color:"), smt_issues_tooltip, change_color_5);
    dlg_data->smt_stats = create_label_line (vbox2, "");
...
{
    const CPUGraph *base = data->base;
    gchar *smt_detected, *smt_issues;

    if (base->topology)
        smt_detected = base->topology->smt ? _("SMT detected: Yes") : _("SMT detected: No");
    else
        smt_detected = _("SMT detected: N/A");

    if (base->topology && (base->topology->smt || base->stats.num_smt_incidents != 0))
    {
        ...
        smt_issues = g_strdup_printf ("%s\n%s\n%s\n\t%s\n\t%s", smt_detected,
                                      lines[0], lines[1], lines[2], lines[3]);
    }
    else
    {
        smt_issues = g_strdup_printf ("%s\n%s", smt_detected, _("Number of SMT scheduling incidents: N/A"));
    }

    if (strcmp (gtk_label_get_text (data->smt_stats), smt_issues))
        gtk_label_set_text (data->smt_stats, smt_issues);
